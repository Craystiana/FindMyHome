...
export class ListingService {
  ...
  getListings(model: ListingQuery): Observable<ListingModel[]> {
    return this.http.post<ListingModel[]>(API_URL + LISTINGS_URL, model).pipe(
  ...
  getOwnListings(): Observable<ListingModel[]> {
    return this.http.get<ListingModel[]>(API_URL + LISTINGS_OWN_URL).pipe(
      map((listings: ListingModel[]) => {
        return listings;
      })
    );
  ...
  getFavouriteListings(): Observable<ListingModel[]>{
    return this.http.get<ListingModel[]>(API_URL + FAVORITE_URL).pipe(
      map((listings: ListingModel[]) => {
        return listings;
      })
    );
  ...
  delete(id: number): Observable<boolean> {
    return this.http.post<boolean>(API_URL + LISTING_DELETE_URL + id, null).pipe(
      map((data: boolean) => {
        return data;
      })
    )
  ...
  edit(model: ListingEdit): Observable<boolean> {
    return this.http.post<boolean>(API_URL + LISTING_EDIT_URL, model).pipe(
      map((data: boolean) => {
        return data;
      })
    )
  ...
  addToFavorites(listingId: number): Observable<boolean> {
    return this.http.post<boolean>(API_URL + FAVORITE_ADD + listingId, null).pipe(
      map((data: boolean) => {
        return data;
      })
    )
  ...
  removeFromFavorites(listingId: number): Observable<boolean> {
    return this.http.post<boolean>(API_URL + FAVORITE_DELETE + listingId, null).pipe(
      map((data: boolean) => {
        return data;
      })
    )
  ...
  getListingData(): Observable<ListingData> {
    return this.http.get<ListingData>(API_URL + LISTING_DATA_URL).pipe(
